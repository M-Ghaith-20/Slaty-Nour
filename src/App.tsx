...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const userDocRef = doc(db, 'users', user.id, 'data', 'main');
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (data.dhikrProgress) setDhikrProgress(data.dhikrProgress);
          if (data.trackerData) setTrackerData(data.trackerData);
          if (data.customDuas) setCustomDuas(data.customDuas);
        }
      } catch (error) {
        handleFirestoreError(error, OperationType.GET, `users/${user.id}/data/main`, user.id);
      } finally {
        setIsSyncing(false);
      }
    };
  ...
  const syncData = async (updates: any) => {
    if (!user) return;
    setIsSyncing(true);
    try {
      const userDocRef = doc(db, 'users', user.id, 'data', 'main');
      ...
      handleFirestoreError(error, OperationType.WRITE, `users/${user.id}/data/main`, user.id);
    } finally {
      setIsSyncing(false);
    }
  };
                  ...
                  {user ? (
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        />
                        <div>
                          <h3 className="font-bold text-stone-800 font-amiri">{user.name}</h3>
                          <p className="text-xs text-stone-500 font-messiri">{user.email}</p>
                        </div>
                      </div>
                      <button 
                    ...
                    <div className="text-center space-y-4">
                      ...
                      <div>
                        <h3 className="font-bold text-stone-800 font-amiri">
                          {lang === 'ar' ? 'سجل دخولك لمزامنة بياناتك' : 'Sign in to sync your data'}
                        </h3>
                        <p className="text-xs text-stone-500 font-messiri mt-1">
                          {lang === 'ar' ? 'احفظ تقدمك في الأذكار وتتبع صلواتك' : 'Save your dhikr progress and prayer tracking'}
                        </p>
                      </div>
                      <button 
                        onClick={handleLogin}
                        className="w-full py-3 bg-white border-2 border-stone-200 rounded-2xl font-bold text-stone-700 hover:bg-stone-50 transition-all flex items-center justify-center gap-3 font-messiri"
                      >
